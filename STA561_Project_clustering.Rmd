...
```{r, message=FALSE}
library(readr)
music = read_csv("data/default_plus_chromatic_features_1059_tracks.txt", col_names = FALSE)
coord = c(ncol(music) - 1, ncol(music))
colnames(music)[coord] = c("lat", "lon")
```
...
sil = rep(NA, n)
for (i in 1:n) {
  clust = kmeans(music[, -coord], i, iter.max = 100)
  sil[i] = mean(silhouette(clust$cluster, dist(x = music[, -coord])))
...
library(factoextra)
library(cluster)
hclust_model = agnes(x = music[, -coord], method = "ward")
...
sil = rep(NA, n)
for (i in 1:n) {
  model = Mclust(music[, -coord], G = i, model = "EII")
  BIC[i] = model$bic
  sil[i] = mean(silhouette(model$classification, dist(x = music[, -coord])))
}
par(mar=c(5, 4, 4, 6))
...
```{r}
fpc::clusterboot(dist(music[, -coord]), B = 1000, bootmethod = "boot", clustermethod = kmeansCBI, k = 5, count = F)
```
```{r}
