...
library(readr)
music = read_csv("data/music_chromatic_with_continents")
non_music = c(ncol(music) - 2, ncol(music) - 1, ncol(music))
```
...
```{r, warning=FALSE, message=FALSE}
model = Mclust(music[, -non_music], G = 9, model = "EII")
library(Rtsne)
tsne <- Rtsne(music[, -non_music], dims = 2, perplexity=30, verbose=TRUE, max_iter = 500)
```
